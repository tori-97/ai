...
class Environment():
    ...
    def step(self, action: str, day: dict):
        ...
        if action == "HOLD":
            ...
            info = f"Position closed with: {self._position.get('profit') if self._position is not None else 0}"
            done = True
        
        if self._position is not None:
            self._steps_open += 1

        if float(self.balance) < float(self.start_balance):
            rewards -= self.start_balance - self.balance
